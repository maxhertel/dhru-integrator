...
class Template {
    ...
    public function page_classes($print = true) {
        ...
        if ($this->remember_theme) {
            $this->page_classes .= ' remember-theme';
        }
    ...
    public function get_tag($print = true, $status = false) {
        ...
        $data = array(
            ...
            '2' => array('class' => 'warning', 'text'  => 'Trial'),
            '3' => array('class' => 'danger', 'text'  => 'Disabled'),
            '4' => array('class' => 'success', 'text'  => 'Completed'),
            '5' => array('class' => 'info', 'text'  => 'Processing'),
            '6' => array('class' => 'warning', 'text'  => 'Pending'),
            '7' => array('class' => 'danger', 'text'  => 'Canceled')
        );

        // Generate tag
        $rand = $status ? rand(4, 7) : rand(0, 3);
        $tag  = '<span class="fs-xs fw-semibold d-inline-block py-1 px-3 rounded-pill bg-' . $data[$rand]['class'] . '-light text-' . $data[$rand]['class'] . '">' . $data[$rand]['text'] . '</span>'. "\n";
